...
export const AuthProvider = ({ children }) => {
    const [auth, setAuth] = useState({ // 인증 상태
        ...
        userNick: localStorage.getItem('userNick') || null, // 로컬 스토리지에서 사용자 닉네임 가져오기
        userId: localStorage.getItem('userId') || null, // 로컬 스토리지에서 사용자 ID 가져오기
    });

    useEffect(() => { // 컴포넌트 마운트 시 실행
        const token = localStorage.getItem('token'); // 로컬 스토리지에서 토큰 가져오기
        const userNick = localStorage.getItem('userNick'); // 로컬 스토리지에서 사용자 닉네임 가져오기
        const userId = localStorage.getItem('userId'); // 로컬 스토리지에서 사용자 ID 가져오기

        if (token && userNick && userId) { // 토큰과 사용자 닉네임, 사용자 ID가 있을 경우
            setAuth({ token, userNick, userId }); // 인증 상태 설정
        }
    }, []);

    const updateAuth = (newAuth) => { // 인증 상태 업데이트 함수
        localStorage.setItem('token', newAuth.token); // 로컬 스토리지에 토큰 저장
        localStorage.setItem('userNick', newAuth.userNick); // 로컬 스토리지에 사용자 닉네임 저장
    ...
    const logout = () => { // 로그아웃
        ...
        setAuth({ token: null, userNick: null, userId: null, profileImage: null }); // 인증 상태 초기화
    };
